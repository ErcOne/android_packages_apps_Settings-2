...
package com.android.settings.auzone;
...
public class Download extends SettingsPreferenceFragment implements OnPreferenceChangeListener {
    ...
    Preference mAuzoneOfficial;
    Preference mAuzoneNightly;
    Preference mPAGapps;
    Preference mKernels;
    Preference mAddons;        
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /* mContest = findPreference("contest_Auzone"); */
        mSupport = findPreference("support_auzone");
        mBanksGapps = findPreference("banks_gapps");
        mAuzoneOfficial = findPreference("auzone_official");
        mAuzoneNightly = findPreference("Auzone_nightly");        
        mPAGapps = findPreference("pa_gapps");
        mKernels = findPreference("Auzone_kernels");
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mSupport) {
            ...
            startActivity(intent);
            return true;
        } else if (preference == mAuzoneOfficial) {
            Uri uri = Uri.parse("http://downloads.Auzoneroms.com/AuzonePop/Official/");
            Intent intent = new Intent(Intent.ACTION_VIEW, uri);
            startActivity(intent);
            return true;
        } else if (preference == mAuzoneNightly) {
            Uri uri = Uri.parse("http://downloads.Auzoneroms.com/AuzonePop/Nightlies/");
            Intent intent = new Intent(Intent.ACTION_VIEW, uri);
            startActivity(intent);
            return true;
        } else if (preference == mKernels) {
            Uri uri = Uri.parse("http://downloads.Auzoneroms.com/Kernels/");
            Intent intent = new Intent(Intent.ACTION_VIEW, uri);
            startActivity(intent);
            return true; 
        } else if (preference == mAddons) {
            Uri uri = Uri.parse("http://downloads.Auzoneroms.com/Add-ons/");
            Intent intent = new Intent(Intent.ACTION_VIEW, uri);
            startActivity(intent);
            return true;                                     
        }
